...
from SAMGeneration.mask_generator_step2 import update_label
        ...
        if (curr_epoch + 1) % 4 == 0:
            ...
            for i, TestDataloader in enumerate(TestDataloaders):
                ...
                with torch.no_grad():
                    ...
                    maes[i] = mae
                net.train(True)
                if TestDataloader.dataset.data_name == 'CAMO' and mae >= 0.062:
                    break

            if maes[0] + maes[1] + maes[2] + maes[3] < best_mae:
                best_mae = maes[0] + maes[1] + maes[2] + maes[3]
