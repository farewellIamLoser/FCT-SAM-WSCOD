...
class SFTA(nn.Module):
    def __init__(self, in_channel, out_channel):
        ...
        self.css1 = CrossStrengthen(out_channel, 1, 3)
        self.css2 = CrossStrengthen(out_channel, 1, 4)
...
class CrossStrengthen(nn.Module):
    def __init__(self, dim, x_stride, y_stride, num_heads=8, bias=False, LayerNorm_type='WithBias'):
        ...
        self.qkvx1conv = nn.Conv2d(dim, dim, kernel_size=3, stride=x_stride, padding=1, groups=dim, bias=bias)
        self.qkvx2conv = nn.Conv2d(dim, dim, kernel_size=3, stride=y_stride, padding=1, groups=dim, bias=bias)
        self.qkvx3conv = nn.Conv2d(dim, dim, kernel_size=3, stride=y_stride, padding=1, groups=dim, bias=bias)
        ...
        self.qkvy1conv = nn.Conv2d(dim, dim, kernel_size=3, stride=y_stride, padding=1, groups=dim, bias=bias)
        self.qkvy2conv = nn.Conv2d(dim, dim, kernel_size=3, stride=x_stride, padding=1, groups=dim, bias=bias)
        self.qkvy3conv = nn.Conv2d(dim, dim, kernel_size=3, stride=x_stride, padding=1, groups=dim, bias=bias)
