...
def get_mask(image, fig_mask, rand):
    np.random.seed(42)
    # get instance mask
    ...
    for i in range(20):
        ...
        mask_image.save('E:\Mr.Wu\dataset\CodDataset\\train\SingleImage\\'+ str(rand)+str(i) +'.png')
...
def get_random_mask(image_files):
    ...
    mask_path = image_files.replace('SingleImage', 'oldScribble').replace('jpg', 'png')
    ...
    if random_number == 0:
        ...
        flipped_mask = get_mask(flipped_image, flipped_mask, random_number)
    ...
    if random_number >= 1 and random_number <= 3:
        ...
        rotated_mask = get_mask(rotated_image, rotated_mask, random_number)
        return_mask = cv2.rotate(rotated_mask, return_angle)

    if random_number == 4:
        return_mask = get_mask(sam_image, mask, random_number)
    ...
    if random_number > 4:
        ...
        normal_mask = get_mask(normal_sam_image, normal_mask, random_number)
...
if __name__ == '__main__':
    np.random.seed(42)
    # read image
    path = r'E:\Mr.Wu\dataset\CodDataset\train\SingleImage'
        ...
        # try:
            for k in range(K):
                if k == 0:
                    ...
                    # final_mask[final_mask > average_non_zero/4] = 255
                    # final_mask[final_mask <= average_non_zero/4] = 0
                else:
                    mask, _ = get_random_mask(image_files)
                    final_mask = mask + final_mask

            final_mask = final_mask.astype(np.uint8)
            maskpath_save = mask_path.replace('oldScribble', 'SingleImage')
