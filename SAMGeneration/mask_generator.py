...
def get_mask(image, fig_mask):
    ...
    if len(fg_coordinates) > 100:
        kernel = np.ones((5, 5), np.uint8)
        fig_mask = cv2.erode(image, kernel, iterations=1)
    # 获取前景点坐标
    fg_row_indices, fg_col_indices = np.where(fig_mask[:, :, 0] == 1)
    fg_coordinates = list(zip(fg_col_indices, fg_row_indices))
...
if __name__=='__main__':
    ...
    for image_files in image_files:
        ...
        try:
            while i < epoch:
                if i == 0:
                    ...
                    final_mask[final_mask < epoch * 2 / 3] = 0
                    final_mask[(final_mask >= epoch * 2 / 3) & (final_mask <= epoch)] = 255
        ...
        maskpath_save = mask_path.replace('oldScribble', 'sam_fg100_bgran_epoch10_0.66')
