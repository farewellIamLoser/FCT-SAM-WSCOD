...
def parse_args():
    ...
    parser.add_argument('--pretrain', type=str,
                        default='E:\Mr.Wu\codes\FSPNet_Weak\checkpoint\Backbone\PVTv2\pvt_v2_b4.pth',
...
def main(args):
    ...
    TestDir = [args.path + r'\TestDataset\CAMO', args.path + r'\TestDataset\COD10K',
               args.path + r'\TestDataset\CHAMELEON', args.path + r'\TestDataset\N4CK']
    ...
    for curr_epoch in range(0, 200):
        ...
        for data in Dataloader:
            count += 1
            img, label, name, strengthen, shape = data['img'].cuda(args.rank), data['label'].cuda(args.rank), data[
                'name'], data['strengthen'], data['shape']
            out1 = net(img)
            ...
            all_loss = all_loss2 + all_loss1 + loss_ssc + loss_gradL1
            m_loss = m_loss2 + m_loss1 + loss_ssc + loss_gradL1
            ...
            if count % 200 == 0 and args.rank == 0:
                print("Epoch:{}, Iter:{}, all_loss:{:.5f}, main_loss:{:.5f}".format(curr_epoch, count,
                                                                                    running_loss_all / count,
                                                                                    running_loss_m / count))
        ...
        if (curr_epoch + 1) % 60 == 0:
            ...
            if maes[0] + maes[1] + maes[3] < best_mae:
                ...
                torch.save({
                    ...
                    'epoch': curr_epoch
                }, r'E:\Mr.Wu\codes\FSPNet_Weak\checkpoint\model_checkpoint' + 'CrossVit.pth')
                print('Save state_dict successfully! Best epoch:{}.'.format(curr_epoch))
            print('the best epoch: {}'.format(best_epoch))
        if args.rank == 0 and (curr_epoch + 1) % 10 == 0:
            ckpt_save_root = "E:\Mr.Wu\codes\FSPNet_Weak\checkpoint\model_checkpoint1"
